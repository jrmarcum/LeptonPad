...
| **Summary**   | same file, `type: 'summary'`     | A Formula variant with a green accent, placeable **anywhere** since 2026-09-21 (v2.2.7 — before that `dropBlock` silently refused a drop outside a section, which read as "the block won't place"). **Only Summary blocks inside a section feed that section's summary** — Formula blocks do not; outside a section a Summary block is just a highlighted formula block. Rows that are comparisons (`sigma < F_b`) become pass/fail entries. |
