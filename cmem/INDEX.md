...
**What is true now, in one paragraph.** LeptonPad is a Deno 2.x browser PWA at **v2.3.34**
...
`tests/` suite of 97 steps runs as part of `deno task check`. Identity is Clerk; roles, license codes, and section-pack ownership live in
...
fall-throughs, initialisation order). Its results, the fixes, and the **six leads that were never
reproduced** are in [`known-issues.md`](known-issues.md) § 19. Start there rather than repeating
those sweeps; the uncovered ground is anything needing a browser.
...
| [architecture.md](architecture.md)             | The project-file format (`.leptonpad`, its top-level keys, and the per-row fields that must round-trip through `syncContent`), the module graph, and the two patterns that hold it together: the mutable-`state.ts` singleton and the callback-slot registry that breaks circular imports. Also the render pipeline and the PWA/service-worker layer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| [blocks.md](blocks.md)                         | Per-row and per-block line spacing, Alt+Arrow cell navigation, the OK/NG result column, and the Math Display font setting (with the old-style-figure trap); The nine block types, what each does, the stretch/resize handle matrix, section blocks and variable scoping (`beam1__L`), summary blocks (section-only and pro+), and custom modules (multi-block user tools).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| [roadmap.md](roadmap.md)                       | Where the project stands at **v2.3.34 (2026-09-23)**, the 14-release math/notation session and the 11-release test-and-unit-correctness session summarised, and what is queued — the `WASM-READY` markers, the section-pack storefront, and what is deliberately not being built.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
