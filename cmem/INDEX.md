...
**What is true now, in one paragraph.** LeptonPad is a Deno 2.x browser PWA at **v2.2.6**
(`deno.json`, 2026-09-21) — a drag-and-drop engineering calculation pad, live at
...
| [math-engine.md](math-engine.md)               | `src/expr.ts` — the tokenizer/parser/evaluator, `Quantity` + `UnitMap` dimensional analysis, compound-unit expansion, `[unit]` vs `[[targetUnit]]`, inline unit tags, the mandatory display-only `\` for Greek/`\sqrt`, backslash repair in hand-written project JSON, affine temperature, control flow, and the `fnScope` `targetUnit` mechanism.                                                                           |
...
| [design-decisions.md](design-decisions.md)     | Why each non-obvious choice was made — TS-not-WASM for math, compound expansion and its display side effect, mutable module state, plot sweep-variable unit propagation, fixed `TITLE_BLOCK_H`, `globalThis` over `window`, mandatory `\` for Greek with no migration of old sheets.                                                                                                                                         |
...
| [known-issues.md](known-issues.md)             | Live defects and traps: `public/config.js` is patched but never shipped (and holds real credentials), version drift, `CLAUDE.md` untracked, the `mathwasm-` localStorage key that cannot be renamed, Windows-only browser launch, `pi`/`e`/`tau` silently shadowing user variables (open), pre-2.2.5 sheets losing Greek display (accepted), the 2.2.6 cross-block audit (`==`, plot dot-notation and sweep-variable fixes). |
